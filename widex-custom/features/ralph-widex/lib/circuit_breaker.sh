...
log_circuit_transition() {
    ...
    # Console log with colors (ASCII-only output)
    case $to_state in
        "$CB_STATE_OPEN")
...
should_halt_execution() {
    ...
    if [[ "$state" == "$CB_STATE_OPEN" ]]; then
        ...
        echo "  2. Check output: ls -lt .ralph/logs/codex_output_*.log | head -1"
        echo "  3. Update .ralph/@fix_plan.md if needed"
        echo "  4. Reset circuit breaker: (delete .ralph/.circuit_breaker_state and re-run)"
        echo ""
