...
- Widex 的当前主线目标只有两个：
  - 尽快、稳定地跟随 `upstream/main`
  - 保持 `ralph-widex` 功能与运行链路稳定
...
- `.ralph/PROMPT.md`：Ralph 提示词（每轮都会要求模型读取）
- `.ralph/@fix_plan.md`：计划（每轮执行前要求读取）
- `.ralph/@fix_progress.md`：进度（推荐 agent 在 Notes 区追加；运行时也会在每轮 start/end 把 auto log 强制落盘，用于“中断后继续/跨轮迭代”）
- `.ralph/.fix_progress.autolog.jsonl`：auto log 的“真源”（append-only）；即使模型误改/覆盖了 `@fix_progress.md`，运行时也能重建 auto log 段落
- `.ralph/STOP`：若存在，请求停止；在当前 turn 结束/中断时生效（推荐直接 `/ralph-widex stop`）
- `.ralph/status.json`：loop 状态（TUI 会 best-effort 写入，便于排查）
...
- 约定：每轮都会更新 `.ralph/@fix_progress.md`，用于“中断后继续/跨轮迭代”的上下文承接
- 固定入口：先 `/ralph-widex init` 初始化当前目录的 `.ralph/` 结构，再编辑模板，然后用 `/ralph-widex start ...` 启动循环
...
阶段 1（历史）：曾保留 Rust 子命令与 shell 版 supervisor 作为迁移过渡层，用于验证 `.ralph/` 模板、状态文件与循环逻辑。

阶段 2（已完成）：TUI `/ralph-widex` 变为 **唯一用户入口**，以原生前台循环方式在同一个 Widex 会话内自动迭代，每轮可见，不再对用户暴露外部 CLI 流程。

阶段 3：继续把状态观察、进度可视化等能力内聚到 TUI 内，而不是依赖额外命令。
...
- structured output：内部每轮会为执行链路自动附带 `--output-schema .ralph/ralph_output_schema.json`，让模型更倾向输出可解析 JSON。
...
- `widex-custom/docs/LLMs_intergration/Grok_intergration.md`
...
- 任何包含真实 secret token 的 yaml/toml/json
