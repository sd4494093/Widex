...
fn run_agent_jobs_test<F, Fut>(name: &'static str, future_factory: F) -> Result<()>
where
...
fn report_agent_job_result_rejects_wrong_thread() -> Result<()> {
    run_agent_jobs_test("report_agent_job_result_rejects_wrong_thread", || async {
...
fn spawn_agents_on_csv_runs_and_exports() -> Result<()> {
    run_agent_jobs_test("spawn_agents_on_csv_runs_and_exports", || async {
        let server = start_mock_server().await;
        let mut builder = test_codex().with_config(|config| {
            config
                .features
...
fn spawn_agents_on_csv_dedupes_item_ids() -> Result<()> {
    run_agent_jobs_test("spawn_agents_on_csv_dedupes_item_ids", || async {
        let server = start_mock_server().await;
...
fn spawn_agents_on_csv_stop_halts_future_items() -> Result<()> {
    run_agent_jobs_test("spawn_agents_on_csv_stop_halts_future_items", || async {
        let server = start_mock_server().await;
        let mut builder = test_codex().with_config(|config| {
            config
                .features
