...
use codex_ralph_widex::Cli as RalphWidexCli;
...
enum Subcommand {
    ...
    /// Update Widex to the latest version.
    Update,
    ...
    /// [EXPERIMENTAL] Ralph autonomous loop (Widex).
    #[clap(name = "ralph-widex")]
    RalphWidex(RalphWidexCli),
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating Widex via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart Widex.");
    Ok(())
}
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    #[cfg(not(debug_assertions))]
    {
        let Some(action) = codex_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the Widex installation method. Please update manually: npm install -g @wellau/widex@latest"
            );
        };
        run_update_action(action)
    }
...
async fn cli_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    match subcommand {
        ...
        Some(Subcommand::RalphWidex(ralph_cli)) => {
            let exe = std::env::current_exe()?;
            codex_ralph_widex::run_main(ralph_cli, exe).await?;
        }
...
fn unsupported_subcommand_name_for_strict_config(
    ...
    match subcommand {
        ...
        Some(Subcommand::RalphWidex(_)) => Some("ralph-widex"),
...
mod tests {
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        ...
        assert!(
            help.contains("Usage: widex plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: widex plugin marketplace add"),
            ("list", "Usage: widex plugin marketplace list"),
            ("upgrade", "Usage: widex plugin marketplace upgrade"),
            ("remove", "Usage: widex plugin marketplace remove"),
        ] {
